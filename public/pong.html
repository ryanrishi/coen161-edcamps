...
{
    px = Math.min(event.pageX + .3*pW, canvas.width - .5*pW);
    pz = Math.min(event.pageY + .3*pD, canvas.height - .5*pD);
    pad.position.x=px;
    pad.position.z=pz;
...
var pad = new THREE.Mesh(
	new THREE.BoxGeometry(pW, pH, pD), pMaterial);
pad.position.y = 0;
pad.position.z = pz;
pad.position.x = px;
scene.add(pad);
